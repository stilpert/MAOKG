...
public class HeaderBitmapImage {
    private short type;
    // тип зображення або сигнатура
    private long size;
    // розмір файлу
    private short reserveField1; // резервоване поле No1
    private short reserveField2; // резервоване поле No2
    private long offset;
    // зміщення
    ...
    // кількість кольорів палітри
    private long numbOfImportantColors; // кількість важливих кольорів
    private long halfOfWidth;
    // половина від ширини зображення (не міститься в
    public HeaderBitmapImage () {}
    // конструктор
    public void setType (short type) // метод "сеттер" для безпечного встановленнязначення поля type
    {
        this.type = type;
    }
