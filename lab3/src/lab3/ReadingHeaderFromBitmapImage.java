...
// та повертатиме об’єкт типу HeaderBitmapImage, з інформацією про зображення
    {
        ...
        while ((line = reader1.read())!=-1) { // поки не кінець файлу
            ...
            if (i==6) //зчитуємо резервоване поле No1
            {
            ...
            if (i==8) //зчитуємо резервоване поле No2
            {
            ...
            if (i==18)
            {
                ...
                half=width;
                if((half%2)!=0)
                // перевірка чи ширина зображення кратна 2 і якщо ні, то збільшуємо це значення на 1
                half++;
                // щоб доповнити значення половини від ширини зображення
                half/=2;
                if((half%4)!=0)
                // якщо не ділиться на 4
                    half=(half/4)*4+4;
                    // доповнюємо значення половини ширини зображення, щоб вона була кратна 4
            }
